...
//! - a one-time [`nonce`](PairingBootstrap::nonce) bound into the pairing
//!   confirmation so a passive observer who later replays the QR cannot complete
//!   a session,
...
//! a QR the user physically scans. The nonce must be freshly random per pairing
...
fn put_field(out: &mut Vec<u8>, field: &[u8]) {
    // Lengths are bounded by realistic name/key/addr sizes; truncation is not a
    // concern for the small fields used here.
